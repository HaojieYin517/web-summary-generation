...
def calculate_score(summary, reference):
    ...
    for sentence in reference:
        ...
        if sentence not in all_scores:
            all_scores[sentence] = sum([words_dict[word.lower()] for word in words])
        else:
            all_scores[sentence] += sum([words_dict[word.lower()] for word in words])
...
def calculate_score1(summary, reference):
    ...
    try:
        ...
        return 0
result = calculate_score1(' '.join(list(summary)), sent_tokenize(text))
print(result)
